...
#define PLAYMOD_VERSION PLAYMOD_VERSION_130
    ...
    */
    typedef enum PrPlaymodReturnValue
    {
        ...
        playmod_BroadcastPrefs =
            17, // Valid during playmod_GetFilePrefs. Tells premiere to call playmod_SetFilePrefs on all open play modules using the returned prefs.
        playmod_CannotRecord = 18, // Returned if recording is requested and the playmod cannot support it.
        playmod_RenderAndPutFrame =
            19, // Put segment could not render this segment, Premiere should render it and return it in a putframe.
    ...
    */
    enum
    {
        ...
        PMCapPutSegment =
            0x00020000, // DEPRECATED in CS4: This playmod can display RTSegments on the hardware (PutFrame functionality with RTSegments)
        PMCapSingleClipPlayerWantsRTSegments = 0x00040000, // This playmod wants segments for single clip players.
        PMCapCanDoFractionalResolution =
            0x00080000, // This playmod can handle fractions of the display resolution for faster playback (called with playmod_SetFractionalResolution)
        PMCapCanPutTemporaryTimeline =
            0x00100000, // This playmod can handle putting a temporary timeline. This is a replacement to the old PutSegment functionality
        PMCapSupportsDisplayStateProperties = 0x00200000, // This playmod supports playmod_SetDisplayStateProperties
        PMCapSupportsOverlayDrawing =
            0x00400000, // This playmod supports drawing overlays through PrSDKPlayModuleOverlaySuite
        PMCapSupportsImmersiveVideo =
            0x00800000, // This playmod supports immersive video through PrSDKImmersiveVideoSuite
    ...
    */
    typedef enum
    {
        ...
        pmPlayerDisplay_Red = 6,
        pmPlayerDisplay_Green = 7,
        pmPlayerDisplay_Blue = 8,
    ...
    */
    ...
    typedef struct
    {
        ...
        float
            safeTitlePercentWidth; // how far (percent-wise) to width inset the overlayed rectangle displaying safe title
        // i.e. for a 720x480 display, "0.1" would mean to inset the rectangle by 72x48 pixels
        float
            safeTitlePercentHeight; // how far (percent-wise) to height inset the overlayed rectangle displaying safe title
        // i.e. for a 720x480 display, "0.1" would mean to inset the rectangle by 72x48 pixels
        prBool enableSafeAction; // kPrTrue = overlay safe action, kPrFalse = disable overlay of safe action
        float
            safeActionPercentWidth; // how far (percent-wise) to width inset the overlayed rectangle displaying safe action
        // i.e. for a 720x480 display, "0.1" would mean to inset the rectangle by 72x48 pixels
        float
            safeActionPercentHeight; // how far (percent-wise) to height inset the overlayed rectangle displaying safe action
        // i.e. for a 720x480 display, "0.1" would mean to inset the rectangle by 72x48 pixels
    ...
    typedef struct
    {
        ...
        PlayModuleDeviceID
            deviceID; // if transmit is true, then this ID will be non-zero to allow use of the DeviceControl suite.
    ...
    */
    ...
    typedef struct
    {
        ...
        PrMemoryPtr
            playmodPrefs; // A pointer to this playmodules preference data if this playmodule matches the editing mode (PrMemory)
        ...
        prFieldType
            fieldType; // field dominance of the file. prFieldsNone, prFieldsUpperFirst, prFieldsLowerFirst, prFieldsUnknown
    ...
    typedef struct
    {
        ...
        csSDK_uint32 pixelAspectDen; // The denominator of the file's pixel aspect ratio
        prFieldType
            fieldType; // New in Pro 3.0. Field dominance. prFieldsNone, prFieldsUpperFirst, prFieldsLowerFirst, prFieldsUnknown
        float audioSampleRate;                                         // New in Pro 3.0
...
#define PRT_VERSION_PLAYABLERANGEREC_CURRENT PRT_VERSION_PLAYABLERANGEREC_PREM7
    ...
    **
    */
    static pmDisplayStateProperties2 ConvertToPmDisplayStateProperties2(
        pmDisplayStateProperties& inPmDisplayStateProperties)
    {
    ...
    */
    typedef struct
    {
        ...
        csSDK_uint32
            fieldOfHorizontalView; // How many degrees of view to display, horizontally, from the entirety of the frame using reverse-projection
        csSDK_uint32
            fieldOfVerticalView; // How many degrees of view to display, vertically, from the entirety of the frame using reverse-projection
        ...
        PrIVAmbisonicsHRIR
            ambisonicsHRIR; // The ID for a head-related impulse response if using PrIVAmbisonicsMonitoring_Binaural
    } pmVRConfiguration;
    ...
    */
    ...
    typedef void (*pmShowFileFrameProxyFunc)(
        PrPlayID playID, csSDK_int32 frametime, int proxywidth, int proxyheight, int dispwidth, int dispheight);
    ...
    typedef void (*pmShowFileFrameWithSafeAreasFunc)(
        PrPlayID playID,
        csSDK_int32 frametime,
        pmDisplayPos* disp,
        prFloatRect* view,
        pmAdornSafeAreasParams* adornSafeAreaParams);
    typedef void (*pmShowFileFrameRenderSettings)(
        PrPlayID playID,
        const int inWidth,
        const int inHeight,
    ...
    */
    enum PrPlaymodSelector
    {
        ...
        playmod_SetDisplayStateProperties =
            84, //deprecates PlayerQuality and FractionalResolution. Param is a const pmDisplayStateProperties*
        ...
        playmod_SetDisplayStateProperties2 =
            96, //deprecates PlayerQuality and FractionalResolution. Param is a const pmDisplayStateProperties2*
