...
public:
	...
	prSuiteError InitializeOpenCL ()
    {
		...
		if (!mKernel)
		{
			...
			result = clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, 0, nullptr, &extensionSize);
			if (result != CL_SUCCESS)
			{
				return suiteError_Fail;
			}
			std::string extensionStr(extensionSize, '\0');
			result = clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, extensionSize, &extensionStr[0], &extensionSize);
			if (result != CL_SUCCESS)
			{
				return suiteError_Fail;
			}
			...
			if (extensionStr.find("cl_khr_fp16") != std::string::npos)
			{
				// define GF_OPENCL_SUPPORTS_16F to enable kernel side half float operations.
				// when half float is supported by the GPU, host app may feed us half float buffers in certain scenarios, such as low quality playback.
				options += " -D GF_OPENCL_SUPPORTS_16F";
			}

			result = clBuildProgram(program, 1, &device, options.c_str(), 0, 0);
			if (result != CL_SUCCESS)
			{
				return suiteError_Fail;
			}

			mKernel = clCreateKernel(program, "kCrossDissolve", &result);
			if (result != CL_SUCCESS)
			{
				return suiteError_Fail;
			}
	...
	prSuiteError InitializeDirectX()
	{
#if HAS_DIRECTX
		...
		if (!sDXContextCache[mDeviceIndex])
		{
			...
			if (!GetShaderPath(L"SDK_CrossDissolve", csoPath, sigPath))
			{
				return suiteError_Fail;
			}
        ...
        if (!sMetalPipelineStateCache[mDeviceIndex])
        {
            @autoreleasepool
            {
                ...
                if (result == suiteError_NoError)
                {
                    ...
                    NSString *name = [NSString stringWithCString:"kCrossDissolve" encoding:NSUTF8StringEncoding];
		...
		PPixHand* outFrame)
	{
		...
		if (mDeviceInfo.outDeviceFramework == PrGPUDeviceFramework_CUDA) {
			...
			params.outPitch = outgoingRowBytes;
			params.inPitch = incomingRowBytes;
			params.destPitch = destRowBytes;
			params.is16f = is16f;
			params.width = width;
			...
			shaderExecution.SetParamBuffer(&params, sizeof(CrossDissolveParams));
			shaderExecution.SetUnorderedAccessView(
				(ID3D12Resource*)destFrameData,
				height * destRowBytes);
			shaderExecution.SetShaderResourceView(
				(ID3D12Resource*)outgoingFrameData,
				height * outgoingRowBytes);
			shaderExecution.SetShaderResourceView(
				(ID3D12Resource*)incomingFrameData,
				height * incomingRowBytes);

			if (!shaderExecution.Execute((UINT)DivideRoundUp(width, 16), (UINT)DivideRoundUp(height, 16))) { return suiteError_Fail; }
			return suiteError_NoError;
#else
			return suiteError_NotImplemented;
#endif // HAS_DIRECTX
