...
////
	...
	*/
		...
		tmVideoMode* outVideoMode)
	{
		...
		/*
		// Below sample code to fetch data in YUV420 10bit PQ

		// Set the color space in which transmit client wants to fetch the data from Host application
		// Please check SDK document to know more about ColorSpaceRec
		
		outVideoMode->outPixelFormat = PrPixelFormat_V210_422_10u_709;
		outVideoMode->outColorSpaceRec.outColorSpaceType = kPrSDKColorSpaceType_SEITags;
		outVideoMode->outColorSpaceRec.outSEICodesRec.colorPrimariesCode = static_cast<csSDK_int32>(PrColorPrimaries::kBT2020);
		outVideoMode->outColorSpaceRec.outSEICodesRec.transferCharacteristicCode = static_cast<csSDK_int32>(PrTransferCharacteristic::kBT2100PQ);
		outVideoMode->outColorSpaceRec.outSEICodesRec.matrixEquationsCode = static_cast<csSDK_int32>(PrMatrixEquations::kBT2020NCL);
		outVideoMode->outColorSpaceRec.outSEICodesRec.isFullRange = false;
		outVideoMode->outColorSpaceRec.outSEICodesRec.isRGB = false;
		outVideoMode->outColorSpaceRec.outSEICodesRec.bitDepth = static_cast<csSDK_int32>(PrEncodingBitDepth::k10u);
		*/
	...
	**
	*/
	tmResult TransmitInstance::StartPushAudio(
		const tmStdParms* 	inStdParms,
		const tmInstance* 	inInstance,
			  PrTime		inStartTime,
			  float			inSpeed,
	...
	{
		// When "push" audio is enabled, the application may send a secondary audio stream
		// to enabled Transmit plug-ins (like for network transmission).
		// This is the place to initialize required resources, e.g. a ring buffer to receive
		// audio data from calls to PushAudio().

		*outSamplesPerFrame = 48000;
		return tmResult_Success;
	}
	...
	**
	*/
	tmResult TransmitInstance::PushAudio(
		const tmStdParms* inStdParms,
		const tmInstance* inInstance,
		const tmPushAudio* inPushAudio)
	{
		// PushAudio will be called periodically if secondary audio is enabled for this plug-in
		// and in the application.
		// It is only called between StartPushAudio() and StopPushAudio().
		//
		// For performance reasons, this API may be called while another API is called on another thread.
		//
		// Note: do not block/wait here, because this will be called on a high priority thread and blocking
		// would prevent other plug-ins to receive and process audio data.
		//
		// Instead, do further processing like network transmission on a worker thread.

		// mRingBuffer->Write(
		//	inPushAudio->inBuffers, // pointers to audio channel data
		//	inPushAudio->inNumChannels,
		//	inPushAudio->inNumSamples);
		// mCurrentPushSamplePosition += inPushAudio->inNumSamples;
	...
	**
	*/
	tmResult TransmitInstance::StopPushAudio(
		const tmStdParms* inStdParms,
		const tmInstance* inInstance)
	{
		// Tear down "push" audio resources:
	
	}
	tmResult TransmitInstance::SetStreamingStateChangedCallback(
		const 	tmStdParms*							inStdParms,
				void*								inContext,
	...
	tmResult TransmitInstance::EnableStreaming(
		const 	tmStdParms* inStdParms,
			 	prBool 		inEnabled)
	{
		mStreamingEnabled = inEnabled;
		return tmResult_Success;
	
	}
		
	tmResult TransmitInstance::IsStreamingEnabled(
		const 	tmStdParms* inStdParms,
				prBool* 	outEnabled)
	{
		*outEnabled = mStreamingEnabled ? kPrTrue : kPrFalse;
		return tmResult_Success;
	
	}
				
	tmResult TransmitInstance::IsStreamingActive(
		const 	tmStdParms* inStdParms,
				prBool* 	outActive)
	{
		*outActive = mStreamingEnabled;
		return tmResult_Success;
	}
...
////
	...
	*/
		...
		tmPluginInfo* outPluginInfo)
	{
		...
		// Set this to kPrTrue if the transmitter can handle "push" (secondary) audio
		// outPluginInfo->outPushAudioAvailable = kPrTrue;
	...
	*/
	...
	tmResult TransmitPlugin::SetStreamingStateChangedCallback(
		const tmStdParms* inStdParms,
		void* inContext,
		tmStreamingStateChangedCallback inCallback)
	{
		// This is called by the app to set a callback which the
		// plug-in has to call whenever the streaming state changes.
		return tmResult_Success;
	}

	tmResult TransmitPlugin::EnableStreaming(
		const tmStdParms* inStdParms,
		prBool inEnabled)
	{
		// Called by the application to enable/disable streaming.
		// The plug-in should stop sending audio and video frames e.g. over
		// a network connection.
		return tmResult_Success;
	}

	tmResult TransmitPlugin::IsStreamingEnabled(
		const tmStdParms* inStdParms,
		prBool* outEnabled)
	{
		// Set *outEnabled to kPrTrue if the plug-in supports streaming
		// (e.g. over network).
		return tmResult_Success;
	}

	tmResult TransmitPlugin::IsStreamingActive(
		const tmStdParms* inStdParms,
		prBool* outActive)
	{
		// Set *outActive to kPrTrue if the plug-in is currently streaming
