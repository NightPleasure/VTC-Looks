...
#define IMPORTMOD_VERSION		IMPORTMOD_VERSION_24
...
typedef struct
{
	...
	csSDK_int32				canProvideClosedCaptions; // Pro 7.0 Importer can provide async closed caption reading
...
// Newer, more easily extensible version of imSelectClipFrameDescriptor.  See imSelectClipFrameDescriptor2.
struct imClipFrameDescriptorRec2 : public imClipFrameDescriptorRec
{
	csSDK_uint32				inDesiredMaxBitDepth;		// PrMaxBitDepth of the requested frame properties
	csSDK_uint32				reserved[45];				// Reserved; set to null by the host.
};
...
enum PrImporterSelector
{
	...
	imGetEmbeddedLUT = 82,
	imSelectClipFrameDescriptor2 = 83,		// Pro 23.2; updated version of imSelectClipFrameDescriptor
	imGetColorSpaceFromOpaqueData = 84 		// Pro 23.3; used to get media color space from the media opaque data
};
...
enum PrImporterReturnValue {
	...
	imHostIORequired = 43,		// Need to read data from file using HostIO
	imHostIOUnsupported = 44,	// HostIO is not supported for this media
	imHostIOFallBackSW = 45,	// Error code indicating the need to read more data for supporting SW fallback using HostIO
...
Required:	Yes, if the Importer reported that it has embedded LUT.
 ...
 Message:	imSelectClipFrameDescriptor2
 Required:	no
			Called to negotiate the clip frame descriptor for render.  Use this version instead of
			imSelectClipFrameDescriptor if the importer needs the new members to make its decision.
			param1 : imFileRef
			param2 : imClipFrameDescriptorRec2*
