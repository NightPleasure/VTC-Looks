...
#define kPFUtilitySuiteVersion12	12
...
typedef struct PF_UtilitySuite11
{
	...
	*/
	...
	/*
	** Get the name of the master clip for the track item at inSequenceTime on inSourceTrack.
	** Set inSourceTrack to -1 to query this track item,
	** or if this is an adjustment layer, to query the top-most track item below
	** this track item, at inSequenceTime.
	*/
	SPAPI PF_Err(*GetMasterClipName)(
		PF_ProgPtr inEffectRef,
		int32_t inSourceTrack,
		PrTime inSequenceTime,
		PrSDKString* outSDKString);
	...
	/*
	** Get the file name of the master clip for the track item at inSequenceTime on inSourceTrack.
	** Set inSourceTrack to -1 to query this track item,
	** or if this is an adjustment layer, to query the top-most track item below
	** this track item, at inSequenceTime.
	** Note: Despite the function name, inSourceTrack is an ID, not an index.
	*/
	SPAPI PF_Err(*GetFileNameByTrackIndex)(
		PF_ProgPtr inEffectRef,
		int32_t inSourceTrack,
		PrTime inSequenceTime,
		PrSDKString* outSDKString);
	...
	/*
	** Get the name of the track item at inSequenceTime on inSourceTrack.
	** Set inSourceTrack to -1 to query this track item,
	** or if this is an adjustment layer, to query the top-most track item below
	** this track item, at inSequenceTime.
	*/
	SPAPI PF_Err(*GetTrackItemName)(
		PF_ProgPtr inEffectRef,
		int32_t inSourceTrack,
		PrTime inSequenceTime,
		PrSDKString* outSDKString);
...
typedef struct PF_UtilitySuite12
{
	SPAPI PF_Err(*GetFilterInstanceID)(
		PF_ProgPtr effect_ref,
		A_long* outFilterInstanceID);
	...
	*/
	...
	/*
	** Is the effect applied to an adjustment layer?
	*/
	SPAPI PF_Err(*IsOnAdjustmentLayer)(
		PF_ProgPtr inEffectRef,
		bool* outIsOnAdjustmentLayer);

	/*
	** Get the track index from the track ID.
	*/
	SPAPI PF_Err(*GetTrackIndexFromID)(
		PF_ProgPtr inEffectRef,
		int32_t inSourceTrackID,
		int32_t* outTrackIndex,
		bool* outIsAudioTrack);

	/*
	** Is the effect applied to a transparent video?
	*/
	SPAPI PF_Err(*IsOnTransparentVideo)(
		PF_ProgPtr inEffectRef,
		bool* outIsOnTransparentVideo);

	/*
	** Get the video frame rate (or audio sample rate if it's an audio-only clip)
	** of the track item at inSequenceTime on inSourceTrack.
	** Set inSourceTrack to -1 to query this track item,
	** or if this is an adjustment layer, to query the top-most track item below
	** this track item, at inSequenceTime.
	*/
	SPAPI PF_Err(*GetVideoFrameRateOrAudioSampleRate)(
		PF_ProgPtr inEffectRef,
		int32_t inSourceTrack,
		PrTime inSequenceTime,
		PrTime* outTicksPerFrame,
		bool* outIsAudioSampleRate);


} PF_UtilitySuite12;

typedef struct PF_UtilitySuite13
{
	SPAPI PF_Err(*GetFilterInstanceID)(
		PF_ProgPtr effect_ref,
		A_long* outFilterInstanceID);
	...
	**	inLayerParamIndex is the index of the layer param.
	**	Its value is the ID of the track being queried.
	*/
	SPAPI PF_Err(*DoesTrackExist)(
		PF_ProgPtr inEffectRef,
		csSDK_uint32 inLayerParamIndex,
		bool* outTrackExists);
