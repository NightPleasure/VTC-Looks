#!/usr/bin/env python3
"""Convert .cube LUT files to C++ source files for VTC Looks plugin.
All LUTs are resampled to 33x33x33 for uniform embedded size."""
import os, sys, glob
...
LOG_ORDER = [
    "Convert Sony", "Dark Forest", "Amethyst", "Low Highlights",
    "Convert Canon", "Convert Fujifilm", "Convert RED",
]
...
def read_cube(filepath):
    ...
    with open(filepath, "r") as f:
        for line in f:
            ...
            parts = s.split()
            if len(parts) >= 3:
                try: data.extend([float(parts[0]),float(parts[1]),float(parts[2])])
                except ValueError: continue
    if dim is None:
        entries = len(data)//3; dim = round(entries**(1.0/3.0))
    return dim, data
...
def sample(dim,data,r,g,b):
    ...
    fr=r*(dim-1); fg=g*(dim-1); fb=b*(dim-1)
    r0=int(fr); g0=int(fg); b0=int(fb)
    r1=min(r0+1,dim-1); g1=min(g0+1,dim-1); b1=min(b0+1,dim-1)
    fr-=r0; fg-=g0; fb-=b0
    def at(ri,gi,bi):
    ...
    c000=at(r0,g0,b0);c100=at(r1,g0,b0);c010=at(r0,g1,b0);c110=at(r1,g1,b0)
    c001=at(r0,g0,b1);c101=at(r1,g0,b1);c011=at(r0,g1,b1);c111=at(r1,g1,b1)
    c00=l3(c000,c100,fr);c10=l3(c010,c110,fr)
    c01=l3(c001,c101,fr);c11=l3(c011,c111,fr)
    c0=l3(c00,c10,fg);c1=l3(c01,c11,fg)
    return l3(c0,c1,fb)

def resample(dim_src,data_src,dim_dst):
    if dim_src==dim_dst: return list(data_src)
    result=[]
    for ri in range(dim_dst):
        r=ri/(dim_dst-1) if dim_dst>1 else 0.
        for gi in range(dim_dst):
            g=gi/(dim_dst-1) if dim_dst>1 else 0.
            for bi in range(dim_dst):
                b=bi/(dim_dst-1) if dim_dst>1 else 0.
                rgb=sample(dim_src,data_src,r,g,b)
                result.extend(rgb)
    return result
...
def load_and_resample(filepath,name):
    ...
    if dim!=TARGET_DIM:
        print(f"-> resample",end=" ",flush=True)
...
def main():
    ...
    with open(log_cpp,"w") as f:
        f.write('#include "../Shared/VTC_LUTData.h"\n\nnamespace vtc {\n\n')
        for name,sname,data in log_luts:
    ...
    with open(rec_cpp,"w") as f:
        f.write('#include "../Shared/VTC_LUTData.h"\n\nnamespace vtc {\n\n')
        for name,sname,data in rec_luts:
    ...
    with open(hdr,"w") as f:
        ...
        for name,_,_ in rec_luts:
            f.write(f'    "{name}",\n')
        f.write("};\n\n")
        log_popup="None|"+"|".join(n for n,_,_ in log_luts)
        rec_popup="None|"+"|".join(n for n,_,_ in rec_luts)
